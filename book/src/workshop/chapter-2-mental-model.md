...
**Workflows** are the building blocks of CRE. As developers, we write **Workflows** using the CRE SDK (available in Go and TypeScript). We use the **CRE CLI** to compile these workflows into WebAssembly (WASM) binaries and simulate them locally before deployment.
...
1. **A Trigger**: An event source that starts a workflow execution (e.g., `cron.Trigger`, `http.Trigger`, `evm.LogTrigger`)
...
x402 uses the HTTP 402 status code (Payment Required) to gate API access. Here's the detailed flow:
...
3. **x402 payment gate**
   ...
   - Client creates EIP-712 payment authorization
...
4. **CRE HTTP trigger receives data**
   - Server sends HTTP POST to CRE workflow
...
2. **Integration points**:
   - x402-protected server endpoint
